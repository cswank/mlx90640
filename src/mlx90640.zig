...
pub fn MLX90640() type {
    return struct {
        ...
        pub fn loadFrame(self: *Self) !void {
            ...
            try self.i2c.writeThenRead(CONTROL_REGISTER, self.frame[832..833]);
        }
...
fn MockI2C() type {
    return struct {
        ...
        pub fn writeThenRead(comm: *I2C(), addr: u16, buf: []u16) !void {
            const self: *Self = @fieldParentPtr("interface", comm);

            const data = try self.getData(addr);
        ...
        fn getData(self: *Self, addr: u16) ![]u16 {
